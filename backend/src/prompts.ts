...
<code_formatting_info>
  ...
  CRITICAL: Generate syntactically valid TSX/JSX. Every opening tag must have the correct matching closing tag. Never emit duplicate adjacent closing tags such as \`</button></button>\`. Never nest \`<button>\` inside another \`<button>\` or \`<a>\` inside another \`<a>\`. Before returning a TSX/JSX file, validate the tag stack from top to bottom.
</code_formatting_info>
